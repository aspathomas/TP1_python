...
X_acp_2factors = coord[:, :2]  # Selecting the first 2 factors
for clf, name_clf in zip(lst_classif, lst_classif_names):
    scores = cross_val_score(clf, X_acp_2factors, y, cv=5)
    print(f"Accuracy of {name_clf} classifier on ACP (2 factors): {scores.mean():.2f} (+/- {scores.std() * 2:.2f})")

print('*** Results for first 2 original variables ACP ***')
for clf, name_clf in zip(lst_classif, lst_classif_names):
    scores = cross_val_score(clf, X_standard, y, cv=5)
    print(f"Accuracy of {name_clf} classifier on original features: {scores.mean():.2f} (+/- {scores.std() * 2:.2f})")
...
for clf, name_clf in zip(lst_classif, lst_classif_names):
    ...
    print(f"Accuracy of {name_clf} classifier on selected features: {scores.mean():.2f} (+/- {scores.std() * 2:.2f})")